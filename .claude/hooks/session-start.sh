...
2. Run `node .claude/hooks/report-html.mjs <report.md>` and show them the HTML page (output/report-*.html), never the Markdown.
...
5. After their answer: generate the tailored CV PDF and display it, show the letter text in chat, and ask « Je la génère en PDF ? (oui / vos modifications) ». On « oui », generate the letter PDF, add "**Cover Letter:** <path>" to the report header, re-run .claude/hooks/report-html.mjs, display the updated page and the CV again, and send the letter.
...
Display rule (mandatory, whole flow): every time the HTML report (output/report-*.html) or the tailored CV PDF is created or updated, display it right away with the file-sending tool (SendUserFile) using display "render", so it opens in the Claude app's side panel (desktop and web). Never deliver these two as a download-only attachment, and never skip them. The cover letter PDF is sent normally. If no file-sending tool is available (e.g. in a terminal), open each of them with the default app (macOS: open <file>; Windows: start "" <file>; Linux: xdg-open <file>) and give the paths. In your message, confirm both are shown; if one could not be displayed, name the file and its path.
EOF
fi
